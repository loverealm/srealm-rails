class Notification
  class << self
    ...
    # mention activities + Custom Activities
    ...
    def recommend_activities_query(user)
      PublicActivity::Activity.includes(:owner)
                              .where(trackable_type: 'Recommend')
                              .joins('join recommends on recommends.user_id = owner_id')
                              .where('activities.owner_id = ?', user.id)
                              .where('activities.recipient_id <> ?', user.id)
                              .to_sql
    end
