...
describe 'api/v1/pub/messages' do
  ...
  describe 'POST api/v1/pub/messages' do
    it 'returns status 201 for successful create' do
      post '/api/v1/pub/messages', access_token: doorkeeper_access_token.token, body: body
    ...
    it 'returns status 422 for unsuccessfull create' do
      post '/api/v1/pub/messages', access_token: doorkeeper_access_token.token, body: nil
    ...
    it 'contains comment fields' do
      ...
      expect(json.keys.sort).to eq %w(id body sender_id removed_at created_at updated_at).sort
  ...
  describe 'DELETE api/v1/pub/messages' do
    let!(:conversation) { FactoryGirl.create(:conversation, new_members: [user1.id, user2.id]) }
